...
export default function ShellSortAnimation() {
    ...
    const handleClick = () => {
        if (!sortComplete) {
            ...
            for (let i = 0; i < sequence.length; i++) {
                if (sequence[i][0] === 'gap') {
                    const gap = sequence[i][1];
                    setTimeout(() => {
                        message[0].innerText = `sorting with gap of ${gap}`;
                    }, i * interval);
                };
                ...
                if (sequence[i][0] === 'completed') {
                    setTimeout(() => {
                        message[0].innerText = 'shell sort completed';
  ...
  return (
    <div>
        <button onClick={handleClick}>Shell sort</button>
    </div>
  )
}
