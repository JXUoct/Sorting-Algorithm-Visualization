...
export default function InsertionSortAnimation() {
    ...
    const {sortedColor, comparingColor, highlightingColor} = useSelector(store => store.color);
        ...
        if (!sortComplete) {
            ...
            for (let i = 0; i < sequence.length; i++) {
                if (sequence[i][0] === 'iterating') {
                    ...
                    setTimeout(() => {
                        message[0].innerText = 'number to insert'
                        bars[b].style.backgroundColor = highlightingColor;
                    }, i * interval);
                };
                if (sequence[i][0] === 'inserting') {
                    ...
                    setTimeout(() => {
                        bars[b1].style.backgroundColor = sortedColor;
                    }, (i + 1/3) * interval);
                };
                if (sequence[i][0] === 'inserted') {
                    const b = sequence[i][1];
                    setTimeout(() => {
                        message[0].innerText = 'inserted'
                        bars[b].style.backgroundColor = sortedColor;
                    }, i * interval);
                };
                if (sequence[i][0] === 'completed') {
                    setTimeout(() => {
                        message[0].innerText = 'insertion sort completed';
